      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[1, 2, 3, 4, 5, 6].map(item => {
            ...
            return (
              <div key={item} className={`${isFirst || isFourth ? 'dark-green-gradient' : isSecond || isFifth ? 'bright-green-gradient' : isThird || isSixth ? 'bright-red-gradient' : 'card-gradient'} rounded-2xl p-8 stroke-border animate-fade-in relative overflow-hidden group hover:scale-105 transition-all duration-300`} style={{
                animationDelay: `${item * 0.1}s`
              }}>
                {/* Card gradient overlay */}
                <div className={`absolute inset-0 opacity-50 bg-gradient-to-br ${isFirst || isFourth ? 'from-green-600/10' : isSecond || isFifth ? 'from-green-400/10' : isThird || isSixth ? 'from-red-400/10' : 'from-primary/10'} to-transparent group-hover:opacity-70 transition-opacity duration-300`}></div>
                
                <div className="relative z-10">
                  <div className={`w-12 h-12 rounded-lg ${isFirst || isFourth ? 'bg-green-600/20' : isSecond || isFifth ? 'bg-green-400/20' : isThird || isSixth ? 'bg-red-400/20' : 'bg-primary/20'} flex items-center justify-center mb-6`}>
                    <div className={`w-6 h-6 rounded ${isFirst || isFourth ? 'bg-green-600' : isSecond || isFifth ? 'bg-green-400' : isThird || isSixth ? 'bg-red-400' : 'bg-primary'}`}></div>
                  </div>
                  <h3 className="text-xl font-semibold text-foreground mb-4">
                    {isFirst || isFourth ? 'Hosting' : isSecond || isFifth ? 'Logo development' : isThird || isSixth ? 'WI-FI & network planning' : `Feature ${item}`}
                  </h3>
